...
import time
from imagernn.utils import initw, initwG, forwardSubRoutine, backMultSubroutine
...
class LSTMGenerator:
  ...
  @staticmethod
  def init(input_size, hidden_size, output_size, mode):
    ...
    if mode == 'CPU':
    	...
    	model['WLSTM'] = initwG(input_size + hidden_size + 1, 4 * hidden_size)
    	model['Wd'] = initwG(hidden_size, output_size) # decoder
    	model['bd'] = gp.zeros((1, output_size))
  ...
  def forward(Xi, Xs, model, params, **kwargs):
    ...
    #tanhC_version = 1 #params.get('tanhC_version', 0)
    #drop_prob_encoder = 0.5 #params.get('drop_prob_encoder', 0.0)
    #drop_prob_decoder = 0.5 #params.get('drop_prob_decoder', 0.0)
    ...
    #dt = time.time() - t0
    #print 'LSTM  init block %.4fs' %(dt)
    #t0 = time.time()
    #print type(n)
    Hin, Hout, IFOG,IFOGf,C = forwardSubRoutine(Hin,Hout, X, WLSTM,IFOG,IFOGf,C,n,d)
    ...
    #dt = time.time() - t0
    #print 'LSTM output block %.4fs' %(dt)
    #t0 = time.time()
